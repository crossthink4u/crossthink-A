...
const quickActions = [
  { label: 'Create New Project', icon: Plus, path: '/dashboard/projects' },
...
export default function CommandPalette({ isOpen, onClose }: { isOpen: boolean; onClose: (action?: string) => void }) {
                ...
                {query.length < 2 && (
                  <div className="p-2">
                    ...
                    {quickActions.map((a, i) => (
                      <button key={i} onClick={() => go(a.path)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/[0.05] text-left transition-colors">
