...
const ApplicationsPage = () => {
  const { applications, projects, showToast } = useDashboard();
  const router = useRouter();

  const openProject = (app) => {
    const match = projects.find((p) => p.id === app.projectId || p.title === app.projectTitle);
    if (match) {
      router.push('/dashboard/projects');
      showToast(`Jump to “${match.title}” from Projects.`);
      return;
    }
    showToast('This project is not on your discover board (demo data).');
  };
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {applications.map((app, idx) => (
                ...
                >
                  <td className="p-4 font-semibold text-gray-900 dark:text-white">{app.projectTitle}</td>
                  <td className="p-4 text-gray-600 dark:text-gray-400 text-sm">{app.role}</td>
                  <td className="p-4 text-gray-600 dark:text-gray-400 text-sm">{app.owner}</td>
